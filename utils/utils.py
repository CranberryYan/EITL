...
def decompose(test_path, test_size):
    ...
    for file in tqdm(flist):
        ...
        # 如果读取失败, 跳过并打印
        if img is None:
            print(f"❌ 跳过：{file} 读取失败！")
            with open("missing_images.txt", "a") as logf:
                logf.write(f"{file} failed to read from {image_path}\n")
            continue
        ...
        for x in range(X - 1):
            ...
            for y in range(Y - 1):
                ...
                cv2.imwrite(os.path.join(path_out, file[:-4] + '_%03d.png' % idx), img_tmp)
                idx += 1
...
def merge(path, path_p, path_s, test_size):
    ...
    missing_log = open("missing_in_merge.txt", "a")
    ...
    for file in sorted(os.listdir(path)):
        ...
        if img is None:
            print(f"❌ 跳过原图：{file}, 无法读取！")
            missing_log.write(f"Missing original image: {file}\n")
            continue
        ...
        for x in range(X - 1):
            ...
            for y in range(Y - 1):
                if y * size // 2 + size > W:
                    break
                tile_path = path_p + file[:-4] + '_%03d.png' % idx
                img_tmp = cv2.imread(tile_path)
                if img_tmp is None:
                    print(f"⚠️ 缺失切片：{tile_path}")
                    missing_log.write(f"Missing tile: {tile_path}\n")
                    idx += 1
                    continue
                ...
                rtn[x * size // 2: x * size // 2 + size, y * size // 2: y * size // 2 + size, :] = \
                    weight_cur * weight_cur + weight_tmp * img_tmp
                idx += 1
