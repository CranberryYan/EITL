...
def load_resume_checkpoint(model, ckpt_path, device):
    """
    断点续训加载：
    - 自动兼容 state_dict 是否带 'module.' 前缀
    - 自动按 shape 对齐加载（不匹配的 key 会跳过）
    返回: checkpoint, start_epoch（下一轮要跑的 epoch）
    """
    ckpt = torch.load(ckpt_path, map_location=device)

    state_dict = ckpt.get("state_dict", ckpt)  # 兼容有人直接存 state_dict
    ...
    for k, v in state_dict.items():
        ...
        if kk in model_dict and model_dict[kk].shape == v.shape:
            new_state[kk] = v
            loaded += 1
        else:
            skipped += 1
    ...
    # 你的 fit_one_epoch 保存的是 "epoch": epoch(0-based)，所以恢复时要 +1
    start_epoch = int(ckpt.get("epoch", -1)) + 1

    print(f"[Resume] Loaded keys: {loaded}, Skipped keys: {skipped}")
    print(f"[Resume] Resume from checkpoint: {ckpt_path}")
    print(f"[Resume] start_epoch (next epoch) = {start_epoch}")
...
if __name__ == "__main__":
    ...
    model_path = r"logs/ep010-loss0.091-val_loss0.174.pth"
    ...
    # ----------------- 数据集路径 -----------------
    data_path = r"/mnt/e/jwj/new_datasets/"
    ...
    # train_txt = os.path.join(data_path, "train_small.txt")
    # val_txt = os.path.join(data_path, "val_small.txt")
