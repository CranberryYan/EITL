...
def test_mode_from_txt(list_path, img_root, dir_save_path):
    """
    根据给定的 txt 列表作为测试集，生成预测 mask 并返回图像级概率字典。

    txt 每行可以是：
      1) 只写基名，例如:  17486
      2) 基名带后缀，例如: 17486.png
      3) 绝对路径，例如:   /mnt/e/jwj/new_datasets/Image/17486.png
      4) 我们之前的三列格式: <img_abs> <mask_abs> <class_name>  ——会取第一列作为图片路径
    """
    if not os.path.isfile(list_path):
        print(f"[Error] 测试列表不存在: {list_path}")
        return {}, {}
    ...
    img_cls_prob = {}   # {save_name: prob_tampered}
    orig_map     = {}   # {base_name: orig_img_path} 方便后面三联图使用

    with open(list_path, "r", encoding="utf-8") as f:
        lines = f.readlines()

    for line in tqdm(lines, desc="Test from txt"):
        raw = line.strip()
        if not raw or raw.startswith("#"):
            continue

        # 取第一个 token，当成图片路径或图片名
        token = raw.split()[0]
        ...
        if os.path.isabs(token):
            ...
            # 2) 相对路径 or 只有文件名
            #    - 若里面带路径分隔符，说明是相对路径：按 img_root 拼接
            #    - 否则认为是基名，到 img_root 下找各种后缀
            if os.path.sep in token:
                img_path = os.path.join(img_root, token)
            else:
                base = os.path.splitext(token)[0]
                img_path = find_original_image(img_root, base)

        if img_path is None or not os.path.exists(img_path):
            print(f"[Warn] 找不到原图: {token}")
            continue

        image = Image.open(img_path)
        base  = os.path.splitext(os.path.basename(img_path))[0]

        # detect_image_resize 返回: seg_img, seg_pred_new, cls_prob
        _, seg_pred, cls_prob = segformer.detect_image_resize(image)
    ...
    print("test_over!")
    return img_cls_prob, orig_map

def evaluate(path_pre, path_gt, dataset_name, record_txt,
             cls_prob_dict=None, orig_map=None):
    # 基本存在性检查
    ...
    for file in tqdm(flist, desc="Evaluate"):
        ...
        try:
            ...
            if orig_map is not None:
                if base in orig_map and os.path.exists(orig_map[base]):
                    orig = cv2.imread(orig_map[base])
...
if __name__ == "__main__":
    used_weigth = r"./datasets=10000+6000_GA=True_BAM=scSE_CAF=CoordMulStrip_Head=FreqHiLoClsHead/logs/ep015-loss0.074-val_loss0.094.pth"
    segformer = SegFormer_Segmentation_GA("b2", used_weigth)

    record_txt = r"./logs/evaluate_result.txt"
    ...
    # 根目录：原图所在位置
    test_root = r"/mnt/e/jwj/new_datasets/Image/"
    # txt 列表：一行一个样本（文件名 / 相对路径 / 绝对路径）
    test_list = r"/mnt/e/jwj/new_datasets/val.txt"

    save_path = r"/mnt/e/jwj/new_datasets/test_result/"
    path_gt   = r"/mnt/e/jwj/new_datasets/Mask_255/"

    rm_and_make_dir(save_path)

    # 用 txt 列表跑推理，拿到 图像级概率 + 原图映射
    cls_prob_dict, orig_map = test_mode_from_txt(test_list, test_root, save_path)

    # 评估 (像素级 + 图像级 + 分桶 + 三联图)
    auc, f1, iou = evaluate(save_path, path_gt, "EITL-Test-Txt",
                            record_txt, cls_prob_dict, orig_map=orig_map)
